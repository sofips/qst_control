...
def create_config():
    ...
    config['learning_parameters'] = {
        'reward_function': 'original',  # Changed from 'site evolution'
        'gamma': '0.95'                 # Changed from '1'
    }
...
def train_ppo(num_episodes=2000, config_path=None, save_dir='./ppo_results'):
    ...
    ppo_config = {
        ...
        'ent_coef': 0.05,                # Increased entropy for exploration
        'max_grad_norm': 0.5,           # Gradient clipping
        'update_epochs': 4,             # Reduced from 10 for stability
        'batch_size': 64,               # Mini-batch size
        'buffer_size': 2048,            # Not used with simple memory
        'hidden_dims': [1024, 1024],      # Network architecture
    ...
    for episode in range(num_episodes):
        ...
        if (episode + 1) % 20 == 0:  # Update every 20 episodes
            ...
            if update_stats:
                ...
                # Debug: Print recent episode performance
                recent_fidelities = [s['max_fidelity'] for s in episode_stats[-20:]]
                print(f"Recent fidelities: [{np.min(recent_fidelities):.4f}, {np.max(recent_fidelities):.4f}], "
                      f"Mean: {np.mean(recent_fidelities):.4f}")
                print(f"Memory size: {len(agent.memory)}")
                
                # Check if we're stuck
                if len(recent_fidelities) >= 20:
                    fidelity_std = np.std(recent_fidelities)
                    if fidelity_std < 0.001:  # Very low variance indicates being stuck
                        print(f"WARNING: Fidelity variance very low ({fidelity_std:.6f}) - agent may be stuck!")

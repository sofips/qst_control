...
class DQNPrioritizedReplay:
    def __init__(
        self,config_instance,sess=None,output_graph=False
    ):

        self.rus = None
        self.n_actions = config_instance.getint("system_parameters", "n_actions")
        self.n_features = config_instance.getint("learning_parameters", "number_of_features")
        self.lr = config_instance.getfloat("learning_parameters", "learning_rate")
        self.gamma = config_instance.getfloat("learning_parameters", "gamma")
        self.epsilon_max = config_instance.getfloat("learning_parameters", "epsilon")
        self.replace_target_iter = config_instance.getint("learning_parameters", "replace_target_iter")
        self.memory_size = config_instance.getint("learning_parameters", "memory_size")
        self.batch_size = config_instance.getint("learning_parameters", "batch_size")
        self.epsilon_increment = config_instance.getfloat("learning_parameters", "epsilon_increment")
        self.fc1_dims = config_instance.getint("learning_parameters", "fc1_dims")
        self.fc2_dims = config_instance.getint("learning_parameters", "fc2_dims")
        self.dropout = config_instance.getfloat("learning_parameters", "dropout")
        self.epsilon = 0 if self.epsilon_increment is not None else self.epsilon_max
        self.prioritized = config_instance.getboolean("learning_parameters", "prioritized_experience_replay")  # decide to use double q or not
        ...
        if output_graph:
            logdirname = "logs_n{}_size{}_per/".format(self.n_features // 2, self.fc1_dims)
    ...
    def _build_net(self):
        ...
        with tf.variable_scope("eval_net", reuse=self.rus):
            ...
            self.q_eval = build_layers(
                self.s, c_names, n_l1, n_l2, w_initializer, b_initializer, True
            )
        ...
        with tf.variable_scope("target_net", reuse=self.rus):
            ...
            self.q_next = build_layers(
                self.s_, c_names, n_l1, n_l2, w_initializer, b_initializer, False
            )

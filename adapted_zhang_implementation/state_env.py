...
import numpy as np
import itertools
from shared import action_selector, state_fidelity, mean_site, gen_props
...
class State(object):
    ...
    def __init__(self, config_instance):
        ...
        action_hamiltonians = action_selector(action_set, self.chain_length,
                                              field_strength, coupling)
    ...
    def step(self, actionnum):
        ...
        reward = self.reward_function(
            next_state, self.stp, config_instance=self.config_instance
        )
        fidelity = state_fidelity(next_state)
        ...
        if noise:
            if np.random.rand() < noise_probability:
                random_phase = np.random.uniform(-1, 1, size=self.chain_length)
                random_phase = random_phase * noise_amplitude
                for i in range(self.chain_length):
                    next_state[i] *= np.exp(1j * random_phase[i])
